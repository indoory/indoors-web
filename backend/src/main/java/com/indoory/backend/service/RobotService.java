...
public class RobotService {
	...
	public ApiDtos.RobotPoseResponse getRobotPose(Long robotId) {
		RobotEntity robot = findRobot(robotId);
		return viewAssemblerService.toRobotPose(robot, findMap(robot.getMapId()), findFloor(robot.getFloorId()));
	}
	...
	@Transactional
	public void dispatch(Long robotId, ApiDtos.DispatchCommandRequest request, SessionOperator operator) {
		RobotEntity robot = findRobot(robotId);
		TaskEntity activeTask = taskService.findActiveTaskForRobot(robot.getId());
		if (activeTask != null) {
		...
		robot.setStatus(RobotStatus.PLANNING);
	...
	public void pause(Long robotId, SessionOperator operator) {
		...
		robot.setStatus(RobotStatus.PAUSED);
		robot.setUpdatedAt(LocalDateTime.now());
		robotRepository.save(robot);

		activityService.recordCommand(robot.getId(), task.getId(), CommandType.PAUSE, "", CommandExecutionStatus.DONE, operator.email());
		activityService.recordEvent(robot.getId(), task.getId(), EventSeverity.WARN, "TASK", "Task " + task.getTaskCode() + " paused by " + operator.email());
		activityService.captureSnapshot(robot);
	}
	...
	public void resume(Long robotId, SessionOperator operator) {
		...
		robot.setStatus(RobotStatus.NAVIGATING);
		robot.setUpdatedAt(LocalDateTime.now());
		robotRepository.save(robot);

		activityService.recordCommand(robot.getId(), task.getId(), CommandType.RESUME, "", CommandExecutionStatus.DONE, operator.email());
		activityService.recordEvent(robot.getId(), task.getId(), EventSeverity.INFO, "TASK", "Task " + task.getTaskCode() + " resumed by " + operator.email());
		activityService.captureSnapshot(robot);
	}
	...
	public void emergencyStop(Long robotId, SessionOperator operator) {
		...
		robot.setStatus(RobotStatus.EMERGENCY_STOP);
		robot.setWarning("Emergency stop engaged");
		...
		activityService.recordCommand(robot.getId(), task == null ? null : task.getId(), CommandType.EMERGENCY_STOP, "", CommandExecutionStatus.DONE, operator.email());
		activityService.recordEvent(
			robot.getId(),
