...
import com.indoory.backend.entity.TaskType;
...
public class TaskService {
	...
	@Transactional
	public ApiDtos.TaskDetailResponse createTask(ApiDtos.CreateTaskRequest request, SessionOperator operator) {
		...
		if (pickup.getId().equals(dropoff.getId())) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Pickup and dropoff must be different locations");
		}

		if (!pickup.getFloorId().equals(dropoff.getFloorId())) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "MVP task creation requires pickup and dropoff on the same floor");
		}
		...
		task.setMapId(map.getId());
		task.setFloorId(floor.getId());
		task.setPickupLocationId(pickup.getId());
		task.setDropoffLocationId(dropoff.getId());
		task.setCurrentStage(TaskStage.QUEUED);
	...
	public void dispatchQueuedTasks() {
		for (TaskEntity queuedTask : taskRepository.findAllByStatusOrderByCreatedAtAsc(TaskStatus.CREATED)) {
	...
	public TaskEntity findActiveTaskForRobot(Long robotId) {
		return taskRepository.findFirstByAssignedRobotIdAndStatusInOrderByCreatedAtDesc(robotId, ACTIVE_TASK_STATUSES)
			.orElse(null);
	}

	private RobotEntity selectRobot(Long mapId, Long floorId) {
		Set<Long> busyRobotIds = taskRepository.findAllByStatusInOrderByCreatedAtDesc(ACTIVE_TASK_STATUSES).stream()
			.map(TaskEntity::getAssignedRobotId)
			.filter(Objects::nonNull)
			.collect(java.util.stream.Collectors.toSet());

		return taskDispatchPolicy.selectRobot(robotRepository.findAllByOrderByIdAsc(), busyRobotIds, mapId, floorId);
	}
	...
	private boolean tryAssignTask(TaskEntity task, String issuedBy) {
		...
		task.setAssignedRobotId(candidate.getId());
		...
		candidate.setStatus(RobotStatus.PLANNING);
		candidate.setUpdatedAt(now);
		candidate.setWarning(null);
	...
	private String nextTaskCode() {
		long nextId = taskRepository.findTopByOrderByIdDesc()
	...
	private ApiDtos.TaskSummaryResponse toTaskSummary(TaskEntity task) {
		...
		LocationEntity pickup = findLocation(task.getPickupLocationId());
		LocationEntity dropoff = findLocation(task.getDropoffLocationId());
		RobotEntity assignedRobot = task.getAssignedRobotId() == null ? null : findRobot(task.getAssignedRobotId());
