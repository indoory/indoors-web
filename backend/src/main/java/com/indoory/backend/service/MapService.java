...
public class MapService {
	...
	public List<ApiDtos.MapMetadataResponse> getMaps() {
		return mapRepository.findAll().stream()
			.map(viewAssemblerService::toMapMetadata)
			.toList();
	}

	@Transactional(readOnly = true)
	public ApiDtos.CurrentMapResponse getCurrentMap() {
		MapEntity map = mapRepository.findFirstByActiveTrue()
	...
	public List<ApiDtos.FloorResponse> getFloors() {
		return floorRepository.findAllByOrderByOrderIndexAsc().stream()
			.map(floor -> viewAssemblerService.toFloor(
				floor,
	...
	public void activate(Long mapId) {
		...
		mapRepository.findAll().forEach(map -> {
			map.setActive(map.getId().equals(target.getId()));
	...
	private ApiDtos.CurrentMapResponse buildMapResponse(MapEntity map) {
		...
		List<ApiDtos.FloorResponse> floorResponses = floors.stream()
			.map(floor -> viewAssemblerService.toFloor(
				floor,
				locationRepository.findAllByFloorIdOrderByIdAsc(floor.getId()).stream()
					.map(location -> viewAssemblerService.toLocation(location, floor))
		...
		List<ApiDtos.MapRobotResponse> robots = robotRepository.findAllByOrderByIdAsc().stream()
			.filter(robot -> robot.getMapId().equals(map.getId()))
			.map(robot -> viewAssemblerService.toMapRobot(
				robot,
		...
		List<ApiDtos.MapTaskResponse> activeTasks = taskRepository.findAllByStatusInOrderByCreatedAtDesc(
			List.of(TaskStatus.ASSIGNED, TaskStatus.RUNNING, TaskStatus.PAUSED, TaskStatus.CREATED)
		).stream()
			.filter(task -> task.getMapId().equals(map.getId()))
			.map(task -> viewAssemblerService.toMapTask(
				task,
				findFloor(task.getFloorId()),
				task.getAssignedRobotId() == null ? null : robotRepository.findById(task.getAssignedRobotId()).orElse(null)
			))
			.toList();
